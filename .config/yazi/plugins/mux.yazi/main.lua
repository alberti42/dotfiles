...
-- Advance the index by `step` but wrap around the count to stay in bounds.
-- Lua's % is floored, so a negative step wraps around to the end.
local function advance_index(current, count, step)
	return ((current - 1 + (step or 1)) % count) + 1
end

-- Cycling direction, taken from the plugin arguments:
--   plugin mux        -> next previewer
--   plugin mux prev   -> previous previewer
-- `previous`, `-1` and `--prev` are accepted too.
local function entry_step(job)
	local args = (job and job.args) or {}
	local word = args[1]
	if word == "prev" or word == "previous" or word == "-1" or args.prev then
		return -1
	end
	return 1
end
...
function M:entry(job)
	...
	local new_current = advance_index(current, previewers_count, entry_step(job))
