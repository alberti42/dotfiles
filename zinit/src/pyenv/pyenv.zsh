...
zinit ice wait'0a' from'gh-r' as'command' lbin'!ccache -> ccache' lucid
zinit light ccache/ccache

# 2. Pyenv plugins
zinit wait'0b' depth=1 light-mode lucid as'null' nocompletions nocompile for \
...
# 3. Pyenv manager
zinit wait'0b' depth'1' light-mode lucid binary \
  atinit"export PYENV_ROOT='$HOME/.pyenv'" \
  atclone"source '${${(%):-%x}:a:h}/__pyenv_atclone_hook.zsh'" \
  atpull"%atclone" \
  atinit"__zcompile_if_needed_and_source zi_pyenv_init.zsh" \
